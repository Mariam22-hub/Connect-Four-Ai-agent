...
def draw_board(board):
    for c in range(COLUMN_COUNT):
        for r in range(ROW_COUNT):
            pygame.draw.rect(screen, burgundy,
...
while not game_over:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            posx = event.pos[0]
            if turn == 0:
                pygame.draw.circle(screen, blue, (posx, int(SQUARE_SIZE / 2)), RADIUS)
            else:
                pygame.draw.circle(screen, violet, (posx, int(SQUARE_SIZE / 2)), RADIUS)
        pygame.display.update()
